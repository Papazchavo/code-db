...
function checkArgs(adapter: unknown, defaultData: unknown) {
  if (adapter === undefined) throw new Error('codedb: missing adapter')
  if (defaultData === undefined) throw new Error('codedb: missing default data')
}

export class Papaz<T = unknown> {
  adapter: Adapter<T>
  data: T
...
export class PapazSync<T = unknown> {
